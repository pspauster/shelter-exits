...
total_exits_dhs <- exits %>%
  ...
  filter(date >= as.Date("2023-06-01"), date <= as.Date("2024-01-01")) %>% #update this range when re-runnign with new data
  group_by(date) %>% 
  summarize(total_exits = sum(exits, na.rm = T))
...
dhs_category %>% 
  filter(date >= as.Date("2023-05-01"), date <= as.Date("2024-01-01")) %>% #update for new data
  pivot_wider(-per_exits, names_from = "category", values_from = "exits") %>% 
