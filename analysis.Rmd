...
entrants_pop_clean <- entrants_pop %>% 
  ...
  pivot_longer(cols = c("families_with_children", "total_single_adults", "adult_families"), names_to = "series", values_to = "value") %>% 
  pivot_wider(names_from = category, values_from = value)

flow_sum <- entrants_pop_clean %>% 
  full_join(total_exits_series %>% filter(series != "runaway_and_homeless_youth"), by = c("date", "series")) %>% 
  ...
  mutate(stayed = population - enter - total_exits)

sankey_data <- flow_sum %>% 
  pivot_longer(cols = c("population", "total_exits", "enter", "stayed")) %>% 
  group_by(name) %>% 
