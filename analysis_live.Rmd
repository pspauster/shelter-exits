...
exits %>% 
  #filter(housing_category == "Permanent Housing", agency == "DHS", date >= as.Date("2023-05-01"), date <= as.Date("2024-01-01")) %>%
  group_by(category) %>% summarize(total = sum(exits, na.rm = T)) %>% 
...
dhs_category %>% 
  #filter(date >= as.Date("2023-05-01"), date <= as.Date("2024-01-01")) %>%  group_by(category) %>% summarize(total = sum(exits, na.rm = T)) %>% 
  adorn_percentages(denominator = "col")
#56%
